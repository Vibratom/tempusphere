...
export function PlatformLink({ name, category, icon: Icon, href, color }: PlatformLinkProps) {
    ...
    return (
        <Link href={href} target={isExternal ? "_blank" : "_self"} rel={isExternal ? "noopener noreferrer" : ""}>
            <div className={cn(
                "group flex h-full flex-col items-center justify-center p-3 rounded-lg text-white transition-all duration-300 transform hover:scale-105 hover:shadow-lg aspect-square",
                color
            )}>
                <Icon className="h-7 w-7 mb-1.5" />
                <span className="font-semibold text-sm text-center">{name}</span>
                <span className="text-xs opacity-80 text-center">{category}</span>
            </div>
        </Link>
    );
}
