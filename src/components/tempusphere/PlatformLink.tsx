...
export function PlatformLink({ name, category, icon: Icon, href, color, description }: PlatformLinkProps) {
    ...
    return (
        <Link href={href} target={isExternal ? "_blank" : "_self"} rel={isExternal ? "noopener noreferrer" : ""}>
            <div className={cn(
                "group flex h-full flex-col items-center justify-center text-center p-4 rounded-lg text-white transition-all duration-300 transform hover:scale-105 hover:shadow-lg w-32 h-24",
                color
            )}>
